...
def tik_primal_dual(meas, f_shape, alpha, show_prog=False):
    ...
    for i in range(2001):
        pk = (pk+sigma*(Af(ft) - meas))/(sigma+1)
        f_prev = fk
        ...
        if show_prog:
            if i % 50 == 0:
                at = -Atf(pk)
                at[at<0] = 0
                Gconj = 1/(4*alpha)*np.linalg.norm(at)**2
                gap = 1/2*np.linalg.norm(Af(fk)-meas)**2 + alpha*np.linalg.norm(fk)**2 + Gconj + np.dot(pk.flatten(),meas.flatten()) +1/2*np.linalg.norm(pk)**2
                pylab.figure(1)
...
def tik_primal_dual_mat(A, At, meas, f_shape, alpha, show_prog=False):
    ...
    for i in range(2001):
        ...
        if show_prog:
            if i % 50 == 0:
                gap = np.linalg.norm(A.dot(ft)-meas_v)**2 + 1/(4)*np.linalg.norm(-At.dot(pk))**2 + np.dot(pk,meas_v) +1/4*np.linalg.norm(pk)**2
                pylab.figure(2)
                pylab.title("Iteration {} | Dual gap: {:.2f}".format(i, gap))
                pylab.gray()
...
if __name__ == "__main__":
    ...
    rec = tik_primal_dual(noisy_data, (n, n), 0.1, True)
